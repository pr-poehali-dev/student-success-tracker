#!/usr/bin/env python3
"""
Generate Excel import example file for classes.
...
def create_excel_file():
    """Create the Excel file using ZIP + XML (XLSX format)"""
    
    # Ensure public directory exists
    os.makedirs("public", exist_ok=True)
    filepath = "public/import_example.xlsx"
    
    # Create XLSX file (which is a ZIP archive with XML files)
...
<sheet name="Классы" sheetId="1" r:id="rId1"/>
        ...
        # xl/sharedStrings.xml - shared strings (all text values)
        xlsx.writestr('xl/sharedStrings.xml', '''<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
...
<si><t>Козлова Елена Владимировна</t></si>
        ...
        # xl/worksheets/sheet1.xml - the actual worksheet data
        # References strings from sharedStrings.xml by index
        # Numbers are stored directly as values
...
if __name__ == "__main__":
    try:
        ...
        print("\nSheet name: Классы")
        print("\nColumns:")
        print("  1. ID класса (text)")
        print("  2. Название класса (text)")
        print("  3. Количество учеников (number)")
        print("  4. Ответственный учитель ID (text)")
        print("  5. Ответственный учитель ФИО (text)")
        print("\nExample data (4 rows):")
        print("  • 5А - 25 students - Иванов Иван Иванович")
        print("  • 6Б - 28 students - Петрова Мария Сергеевна")
        print("  • 7В - 23 students - Сидоров Алексей Петрович")
        print("  • 8Г - 26 students - Козлова Елена Владимировна")
        print("\n" + "=" * 60)
        
    except Exception as e:
        print(f"ERROR: Failed to create Excel file: {e}", file=sys.stderr)
        sys.exit(1)
